        ...
        private void save()
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                {
                    ...
                    SqlCommand cmd4 = new SqlCommand("update tblDRitemCode set productcode=@productcode where productcode= '" + productcode + "'", itemCode);
                ...
                {
                    itemCode.Open();
                    SqlCommand cmd4 = new SqlCommand("update itemCode set productcode=@productcode where productcode= '" + productcode + "'", itemCode);
                    cmd4.Parameters.AddWithValue("@productcode", fprcode);
                    cmd4.ExecuteNonQuery();
                    itemCode.Close();
                }
