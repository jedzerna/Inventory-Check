    ...
    public partial class bodegabreakdown : Form
    {
        ...
        private void bodegabreakdown_Load(object sender, EventArgs e)
        {
            ...
            if (dataGridView2.Rows.Count != 0)
            {
                ...
                {
                    sum += Math.Round((decimal)Convert.ToDecimal(dataGridView2.Rows[i].Cells[3].Value), 2);
                }
        ...
        private void guna2Button6_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (guna2ComboBox3.Text == rows.Cells[2].Value.ToString())
                    {
                ...
                if (found == false)
                {
                    ...
                    else
                    {
                        ...
                        row[0] = 0;
                        row[1] = id;
                        row[2] = guna2ComboBox3.Text;
                        row[3] = guna2NumericUpDown1.Text;
                        dt.Rows.Add(row);
                    }
            ...
            //dataGridView2.Rows[n].Cells[0].Value = id;
            //dataGridView2.Rows[n].Cells[1].Value = "";
            //dataGridView2.Rows[n].Cells[2].Value = "";
            ...
            //add += sum + Convert.ToDouble(guna2NumericUpDown1.Text);

            //if (add > Convert.ToDouble(label2.Text))
            //{
            //    MessageBox.Show("The amount of stocks reach its limits");
            //}
            //else
            ...
            //        insCmd.Parameters.AddWithValue("@qty", guna2NumericUpDown1.Text);
            ...
            //    {
            //        bodega += row.Cells[2].Value.ToString() + ", ";
            //    }
        ...
        private void dataGridView2_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView2.Rows.Count != 0)
            {
                ...
                {
                    MessageBox.Show("The amount of stocks reach its limits");
                    dataGridView2.CurrentRow.Cells[3].Value = "0.00";
                }
            }
        }
        ...
        private void guna2Button3_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //foreach (DataGridViewRow rows in dataGridView2.Rows)
                //{
                //    if ("0" == rows.Cells[0].Value.ToString())
                //    {
            ...
            else
            {
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@itemid", row.Cells[1].Value.ToString());
                        cmd.Parameters.AddWithValue("@bodega", row.Cells[2].Value.ToString());
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    if (row.Cells[0].Value.ToString() == "0")
                    {
                        ...
                        using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                        {
                            ...
                            insCmd.Parameters.AddWithValue("@itemid", row.Cells[1].Value.ToString());
                            insCmd.Parameters.AddWithValue("@bodega", row.Cells[2].Value.ToString());
                            insCmd.Parameters.AddWithValue("@qty", row.Cells[3].Value.ToString());
                            int affectedRows = insCmd.ExecuteNonQuery();
                            itemCode.Close();
                        }
                    }
