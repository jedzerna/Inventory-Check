    ...
    public partial class outlist : Form
    {
        ...
        public void loadprein()
        {
            ...
            using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
            {
                ...
                dbDR.Open();
                string list = "Select drnumber,datetime,projectname,operation,Id,itemcode,sv,dateentered from tblDR";
        ...
        public void loadprein2()
        {
            using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
            {
                ...
                string list = "Select drnumber,datetime,projectname,operation,Id,itemcode,dateentered from tblDR order by Id desc" +
                    "";
                SqlCommand command = new SqlCommand(list, dbDR);
                SqlDataReader reader = command.ExecuteReader();
                dt.Load(reader);
