        ...
        private void printPreviewVP_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView2.Rows)
            {
                ...
                using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                {
                    ...
                    using (SqlDataAdapter a2 = new SqlDataAdapter("Select si,qty,unit,description,cost,total,charging from itemCode where poid = '" + row.Cells["POid"].Value.ToString() + "' order by Id asc", itemCode))
                        a2.Fill(dt);
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            for (int i = 0; i < stringLength; i += chunkSize)
                            {
                                ...
                                if (jed == 1)
                                {
                                    ...
                                    MyPrinter.Print((char)15 + "           " + si.PadRight(12, ' ') + qty.PadLeft(10, ' ') + "  " + unit.PadRight(3, ' ') + "  " + desc.Substring(i, chunkSize).PadRight(34, ' ') + " " + uprice.PadLeft(16, ' ') + " " + amount.PadLeft(19, ' ') + " " + charging.PadRight(14, ' ') + " " + reference.PadRight(6, ' ') + "\r\n");
                                    lines++;
                                }
                                else
                                {
