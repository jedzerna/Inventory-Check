    ...
    public partial class DRForm : Form
    {
        ...
        private void color()
        {
            if (Properties.Settings.Default.color == "true")
            {
                ...
                guna2TextBox6.FillColor = Color.FromArgb(34, 35, 35);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
         ...
         private void save()
            ...
            {
                dbDR.Open();
                string insStmt = "insert into tblDR ([drnumber], [additionalinfo], [itemcode], [projectcode], [projectname], [datetime], [operation], [qty], [totalitems], [totalamount], [createdby], [sv], [dateentered], [subcode], [subname]) values" +
                 " (@drnumber,@additionalinfo,@itemcode,@projectcode,@projectname,@datetime,@operation,@qty,@totalitems,@totalamount,@createdby,@sv,@dateentered,@subcode,@subname)";
                ...
                insCmd.Parameters.AddWithValue("@subcode", guna2TextBox1.Text);
                insCmd.Parameters.AddWithValue("@subname", guna2TextBox6.Text);
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn &&
                e.RowIndex >= 0)
            {
                //if (dataGridView1.CurrentRow.Cells["Column2"].Value.ToString() == "")
                //{
        ...
        private void dataGridView3_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                if (row.Cells["dataGridViewTextBoxColumn17"].Value.ToString() == dataGridView3.CurrentRow.Cells["dataGridViewTextBoxColumn8"].Value.ToString())
                {
                    exist = true;
                    break;
                }
        ...
        private void add()
        {
            ...
            dataGridView1.Rows[n].Cells["Column2"].Value = "";
     ...
     && e.KeyChar != '.')
        ...
        private void guna2Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                if (guna2ComboBox3.Text == "Direct Sales")
                {
                    if (guna2TextBox1.Text == ""|| guna2TextBox6.Text == "")
                    {
                        MessageBox.Show("Please enter the Subcode of the customer and the name.");
                        return;
                    }
                }
                if (guna2TextBox3.Text != "")
        ...
        public void addition()
        {
            using (SqlConnection codeMaterial = new SqlConnection(ConfigurationManager.ConnectionStrings["codeMaterial"].ConnectionString))
            {
                ...
                codeMaterial.Open();
                string listA = "Select product_code,mfg_code,description,ID,stocksleft,cost,selling,dept,unit from codeMaterial WHERE ID BETWEEN '" + idindt + "' AND '" + maxId + "'";
                ...
                foreach (DataRow rows in dat.Rows)
                {
                    ...
                    if (check != 1)
                    {
                        ...
                        newRow[0] = rows["product_code"].ToString();
                        newRow[1] = rows["mfg_code"].ToString();
                        newRow[2] = rows["description"].ToString();
                        newRow[3] = rows["ID"].ToString();
                        newRow[4] = rows["stocksleft"].ToString();
                        newRow[5] = rows["cost"].ToString();
                        newRow[6] = rows["selling"].ToString();
                        newRow[7] = rows["dept"].ToString();
                        newRow[8] = rows["unit"].ToString();
        ...
        private void DRForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (thread != null)
            {
                thread.Abort();
        ...
        private void guna2TextBox5_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Down)
            {
                ...
                {
                    rowIndex = dataGridView3.SelectedCells[0].OwningRow.Index;
                    ...
                    if (rowIndex < dataGridView3.Rows.Count - 1)
                    {
                        ...
                        this.dataGridView3.Rows[rowIndex + 1].Cells[col].Selected = true;
                    }
                }
            }
            else if (e.KeyCode == Keys.Up)
            {
                ...
                if (dataGridView3.SelectedRows.Count > 0)
                {
                    ...
                    if (rowIndex >= 1)
                    {
                        dataGridView3.FirstDisplayedScrollingRowIndex = rowIndex - 1;
                    }
                    if (rowIndex > 0)
                    {
                        dataGridView3.ClearSelection();
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                if (dataGridView3.SelectedRows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        //int a = dataGridView3.CurrentRow.Cells["dataGridViewTextBoxColumn8"].ColumnIndex;
                        if (row.Cells["dataGridViewTextBoxColumn17"].Value.ToString() == dataGridView3.SelectedCells[dataGridView3.CurrentRow.Cells["dataGridViewTextBoxColumn8"].ColumnIndex].Value.ToString())
                        {
                            exist = true;
                            break;
                        }
        ...
        private void addselected()
        {
            ...
            //int n = dataGridView1.Rows.Add();
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn14"].Value = dataGridView3.SelectedCells[0].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn15"].Value = dataGridView3.SelectedCells[1].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn16"].Value = dataGridView3.SelectedCells[2].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn17"].Value = dataGridView3.SelectedCells[3].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn18"].Value = "0.00";
            using (SqlConnection codeMaterial = new SqlConnection(ConfigurationManager.ConnectionStrings["codeMaterial"].ConnectionString))
            {
                codeMaterial.Open();
                DataTable dt = new DataTable();
                String query = "SELECT stocksleft FROM codeMaterial WHERE ID = '" + dataGridView3.SelectedCells[3].Value.ToString() + "'";
                SqlCommand cmd = new SqlCommand(query, codeMaterial);
                SqlDataReader rdr = cmd.ExecuteReader();
            ...
            //dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn20"].Value = dataGridView3.CurrentRow.Cells[4].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn21"].Value = dataGridView3.SelectedCells[5].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn22"].Value = dataGridView3.SelectedCells[6].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn23"].Value = dataGridView3.SelectedCells[7].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn19"].Value = dataGridView3.SelectedCells[8].Value;
            dataGridView1.Rows[n].Cells["dataGridViewTextBoxColumn24"].Value = "0.00";
            dataGridView1.Rows[n].Cells["Column1"].Value = "0.00";
            dataGridView1.Rows[n].Cells["Column2"].Value = "";
