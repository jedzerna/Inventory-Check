    ...
    public partial class printViewDRSum : Form
    {
        ...
        private void printViewDRSum_Load(object sender, EventArgs e)
        {
            ...
            using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
            {
                ...
                if (incom == true)
                {
                    ...
                    list = "SELECT drnumber,datetime,projectname,itemcode FROM tblDR WHERE datetime BETWEEN @date1 AND @date2 AND sv = '" + sv + "' ORDER BY datetime,drnumber ASC";
                }
                else
                {
                    //MessageBox.Show("False");
                    list = "SELECT drnumber,datetime,projectname,itemcode FROM tblDR WHERE datetime BETWEEN @date1 AND @date2 AND operation = 'Completed' AND sv = '" + sv + "' ORDER BY datetime,drnumber ASC";
                }
        ...
        private void print()
        {
            ...
            using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
            {
                ...
                dbDR.Open(); string list = "";
                if (incom == true)
                {
                    //MessageBox.Show("True");
                    list = "SELECT drnumber,datetime,projectname,itemcode,totalamount FROM tblDR WHERE datetime BETWEEN @date1 AND @date2 AND sv = '" + sv + "' ORDER BY datetime,drnumber ASC";
                }
                else
                {
                    //MessageBox.Show("False");
                    list = "SELECT drnumber,datetime,projectname,itemcode,totalamount FROM tblDR WHERE datetime BETWEEN @date1 AND @date2 AND operation = 'Completed' AND sv = '" + sv + "' ORDER BY datetime,drnumber ASC";
                }
                SqlDataAdapter command = new SqlDataAdapter(list, dbDR);
                command.SelectCommand.Parameters.AddWithValue("@date1", Convert.ToDateTime(from.ToString("yyyy-MM-dd")));
                command.SelectCommand.Parameters.AddWithValue("@date2", Convert.ToDateTime(to.ToString("yyyy-MM-dd")));
                ...
                foreach (DataRow item in tblsi.Rows)
                {
                    ...
                    //dataGridView1.Rows[a].Cells["project"].Value = item["projectname"].ToString();
                    checklines();
                    MyPrinter.Print((char)15 + item["drnumber"].ToString().ToString().PadLeft(17, ' ') + "  " + item["projectname"].ToString().ToUpper() + "\r\n");
                    lines++;
