...
namespace Inventory_Check
{
    public partial class itemdetail : Form
    {
        ...
        private void itemdetail_Load(object sender, EventArgs e)
        {
            ...
            {
                if (row.Cells["TYPE"].Value.ToString().Contains("Deleted "))
                {
                ...
                else
                {
                    ...
                    if (row.Cells["TYPE"].Value.ToString() == "PO" && row.Cells["reference1"].Value.ToString() != "Deleted ")
                    {
                        guna2Button7.Visible = false;
                        return;
                    }
            ...
            //{
            //    itemCode.Open();
            //    SqlCommand check_User_Name = new SqlCommand("SELECT COUNT(*) FROM [tblDRitemCode] WHERE ([iitem] = @iitem)", itemCode);
            //    check_User_Name.Parameters.AddWithValue("@iitem", id);
            ...
            //{
            //    itemCode.Open();
            //    SqlCommand check_User_Name = new SqlCommand("SELECT COUNT(*) FROM [itemCode] WHERE ([iitem] = @iitem)", itemCode);
            //    check_User_Name.Parameters.AddWithValue("@iitem", id);
            //    int UserExist = (int)check_User_Name.ExecuteScalar();
        ...
        {
            decimal totalincoming = 0.00M;
            DataTable dt = new DataTable();
            using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
            {
                itemCode.Open();
                string list = "Select ponumber,qty,cost,total,typeofp,icode from itemCode where iitem = '" + id + "' order by Id desc";
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    {
                        totalincoming += Convert.ToDecimal(row["qty"].ToString());
                        using (SqlConnection tblIn = new SqlConnection(ConfigurationManager.ConnectionStrings["tblIn"].ConnectionString))
                        {
                            tblIn.Open();
                            String query = "SELECT suppliername,datetime FROM tblIn WHERE itemcode = '" + row["icode"].ToString() + "'";
            ...
            label15.Text = string.Format("{0:#,##0.00}", totalincoming);
        }
        public void loadoutcoming()
        {
            decimal totaloutcoming = 0.00M;
            DataTable dt = new DataTable();
            using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
            {
                itemCode.Open();
                string list = "Select drnumber,projectname,qty,selling,total,typeofp,icode from tblDRitemCode where iitem = '" + id + "' order by Id desc";
                SqlCommand command = new SqlCommand(list, itemCode);
                SqlDataReader reader = command.ExecuteReader();
                dt.Load(reader);
                itemCode.Close();
                ...
                foreach (DataRow row in dt.Rows)
                {
                    if (row["typeofp"].ToString() == "" || row["typeofp"] == DBNull.Value)
                    {
                        totaloutcoming += Convert.ToDecimal(row["qty"].ToString());
                        using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
                        {
                            dbDR.Open();
                            String query = "SELECT datetime,drnumber FROM tblDR WHERE itemcode = '" + row["icode"].ToString() + "'";
                            SqlCommand cmd = new SqlCommand(query, dbDR);
                            SqlDataReader rdr = cmd.ExecuteReader();
                            if (rdr.Read())
                            {
                                row["drnumber"] = rdr["drnumber"].ToString();
                                row["datetime"] = DateTime.Parse(rdr["datetime"].ToString()).ToString("MM/dd/yyyy");
                            }
                            dbDR.Close();
                        }
            ...
            label16.Text = string.Format("{0:#,##0.00}", totaloutcoming);
        ...
        //        itemCode.Open();
        //        DataTable dt = new DataTable();
        //        string list = "Select * from itemCode where iitem = '" + id + "' AND typeof = 'PO' order by id desc";
        //        SqlCommand command = new SqlCommand(list, itemCode);
        //        SqlDataReader reader = command.ExecuteReader();
        //        dt.Load(reader);
        //        itemCode.Close();
        ...
        public void load()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                string peritemid = row["peritemid"].ToString();
                string tblhistoryid = row["id"].ToString();
                if (row["type"].ToString() == "PO")
                {
                    using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                    {
                        ...
                        if (peritemid == "")
                        {
                            query = "SELECT ponumber,qty,stocksleft,cost,total FROM itemCode WHERE poid = '" + ponumber + "' AND iitem = '" + row["itemid"].ToString() + "'";
                        }
                        else
                        {
                            query = "SELECT ponumber,qty,stocksleft,cost,total FROM itemCode WHERE poid = '" + ponumber + "' AND iitem = '" + row["itemid"].ToString() + "' AND Id = '" + peritemid + "'";
                        }
                        ...
                        else
                        {
                            ...
                            using (SqlConnection tblIn = new SqlConnection(ConfigurationManager.ConnectionStrings["tblIn"].ConnectionString))
                            {
                                ...
                                tblIn.Open();
                                String queryss = "SELECT ponumber FROM tblIn WHERE Id = '" + ponumber + "'";
                                SqlCommand cmdaa = new SqlCommand(queryss, tblIn);
                                SqlDataReader rdsssr = cmdaa.ExecuteReader();

                                if (rdsssr.Read())
                                {
                                    podr = "Deleted " + "PO: " + (rdsssr["ponumber"].ToString());
                                }
                                tblIn.Close();
                    ...
                    object[] o = { date, ponumber, podr, vp, add, sub, total, selling, cost, totalamount, type, tblhistoryid };
                    dttt.Rows.Add(o);
                }
                else if (row["type"].ToString() == "DR")
                {
                    using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                    {
                        ...
                        if (peritemid == "")
                        {
                            query = "SELECT drnumber,qty,stocksleft,selling,total FROM tblDRitemCode WHERE drid = '" + ponumber + "' AND iitem = '" + row["itemid"].ToString() + "'";
                        }
                        else
                        {
                            query = "SELECT drnumber,qty,stocksleft,selling,total FROM tblDRitemCode WHERE drid = '" + ponumber + "' AND iitem = '" + row["itemid"].ToString() + "' AND Id = '" + peritemid + "'";
                        }
                        SqlCommand cmd = new SqlCommand(query, itemCode);
                        SqlDataReader rdr = cmd.ExecuteReader();
                        if (rdr.Read())
                        ...
                        else
                        {
                            ...
                            using (SqlConnection dbDR = new SqlConnection(ConfigurationManager.ConnectionStrings["dbDR"].ConnectionString))
                            {
                                //dataGridView1.Rows.Clear();
                                dbDR.Open();
                                String queryss = "SELECT drnumber FROM tblDR WHERE Id = '" + ponumber + "'";
                                SqlCommand cmdaa = new SqlCommand(queryss, dbDR);
                                SqlDataReader rdsssr = cmdaa.ExecuteReader();

                                if (rdsssr.Read())
                                {
                                    podr = "Deleted " + "DR: " + (rdsssr["drnumber"].ToString());
                                }
                                dbDR.Close();
                        ...
                        object[] o = { date, ponumber, podr, vp, add, sub, string.Format("{0:#,##0.00}", total), selling, cost, totalamount, type , tblhistoryid };
                        dttt.Rows.Add(o);
                    }
                }
                else
                {
                    if (row["podrid"].ToString() == "")
                    {
                        ponumber = row["id"].ToString();
                    }
                    else
                    {
                        ponumber = row["podrid"].ToString();
                    }
                    podr = "";
                    vp = "";
                    add = row["aqty"].ToString();
                    ...
                    if (row["type"].ToString() == "DDL")
                    {
                        ...
                        //    //dataGridView1.Rows.Clear();
                        //    codeMaterial.Open();
                        //    String query = "SELECT * FROM tblHistory WHERE id = '" + row["id"].ToString() + "'";
        ...
        {
            MessageBox.Show(dataGridView3.Rows[1].Cells["TOTAL"].Value.ToString());
            MessageBox.Show(dataGridView3.Rows[2].Cells["TOTAL"].Value.ToString());
            //MessageBox.Show(dataGridView3.Rows[3].Cells["TOTAL"].Value.ToString());
            //MessageBox.Show(dataGridView3.Rows[4].Cells["TOTAL"].Value.ToString());
            MessageBox.Show(dataGridView3.Rows[5].Cells["TOTAL"].Value.ToString());
            MessageBox.Show(dataGridView3.Rows[6].Cells["TOTAL"].Value.ToString());
            MessageBox.Show(dataGridView3.Rows[7].Cells["TOTAL"].Value.ToString());
            MessageBox.Show(dataGridView3.Rows[8].Cells["TOTAL"].Value.ToString());
        }
        ...
        private void guna2Button4_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                using (SqlConnection codeMaterial = new SqlConnection(ConfigurationManager.ConnectionStrings["codeMaterial"].ConnectionString))
                {
                    ...
                    cmd.Parameters.AddWithValue("@description", guna2TextBox3.Text.Trim());
                    ...
                    codeMaterial.Open();
                    string insStmt = "insert into codeMaterialHistory ([product_code], [mfg_code], [description], [stocksleft], [cost], [selling], [unit], [date], [modifiedby], [remarks], [iitem]) values" +
                        " (@product_code,@mfg_code,@description,@stocksleft,@cost,@selling,@unit,@date,@modifiedby,@remarks,@iitem)";
                    ...
                    insCmd.Parameters.AddWithValue("@description", guna2TextBox3.Text.Trim());
                    ...
                    insCmd.Parameters.AddWithValue("@iitem", id);
                    int affectedRows = insCmd.ExecuteNonQuery();
                    codeMaterial.Close();

                    using (SqlConnection itemCode = new SqlConnection(ConfigurationManager.ConnectionStrings["itemCode"].ConnectionString))
                    {
                        SqlCommand cmd2 = new SqlCommand("update itemCode set productcode=@productcode,mfgcode=@mfgcode, description=@description,unit=@unit where iitem='" + id + "'", itemCode);
                        itemCode.Open();
                        ...
                        cmd2.Parameters.AddWithValue("@description", guna2TextBox3.Text.Trim());
                        cmd2.Parameters.AddWithValue("@unit", guna2TextBox5.Text.Trim());
                        cmd2.ExecuteNonQuery();
                        itemCode.Close();

                        SqlCommand cmd2a = new SqlCommand("update tblSIitems set productcode=@productcode,mfgcode=@mfgcode, description=@description,unit=@unit where iitem='" + id + "'", itemCode);
                        itemCode.Open();
                        cmd2a.Parameters.AddWithValue("@productcode", guna2TextBox1.Text);
                        cmd2a.Parameters.AddWithValue("@mfgcode", guna2TextBox2.Text.Replace("[", lb).Replace("]", rb).Replace("​*", "[*​]").Replace("%", "[%]").Replace("'", "''"));
                        cmd2a.Parameters.AddWithValue("@description", guna2TextBox3.Text.Trim());
                        cmd2a.Parameters.AddWithValue("@unit", guna2TextBox5.Text.Trim());
                        cmd2a.ExecuteNonQuery();
                        itemCode.Close();

                        SqlCommand cmd3 = new SqlCommand("update tblDRitemCode set productcode=@productcode,mfgcode=@mfgcode,description=@description,unit=@unit where iitem='" + id + "'", itemCode);
                        itemCode.Open();
                        ...
                        cmd3.Parameters.AddWithValue("@description", guna2TextBox3.Text.Trim());
                        cmd3.Parameters.AddWithValue("@unit", guna2TextBox5.Text.Trim());
        ...
        private void guna2Button7_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView3.Rows)
            {
                if (row.Cells["TYPE"].Value.ToString() == "DR")
                {
                    MessageBox.Show("Error: Item is already in one of the transactions.");
                    guna2Button7.Visible = false;
                    return;
                }
                if (row.Cells["TYPE"].Value.ToString() == "PO" && row.Cells["reference1"].Value.ToString() != "Deleted")
        ...
        private void dataGridView3_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 7 && e.RowIndex != this.dataGridView3.NewRowIndex)
            {
                if (dataGridView3.Rows[e.RowIndex].Cells["TOTAL"].Value != null)
                {
                    if (e.Value.ToString() != "")
                    {
            ...
            foreach (DataGridViewRow row in dataGridView3.Rows)
            {
                ...
                else if (row.Cells["TYPE"].Value.ToString() == "PDL" || row.Cells["TYPE"].Value.ToString() == "DDL")
                {
        ...
        private void po()
        {
            ...
            using (SqlConnection tblIn = new SqlConnection(ConfigurationManager.ConnectionStrings["tblIn"].ConnectionString))
            {
                if (dataGridView3.CurrentRow.Cells["ponumber"].Value.ToString() == "Deleted")
                {
                    MessageBox.Show("Record is deleted", "Deleted", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    tblIn.Open();
        ...
        private void elsedata()
        {
            ...
            {
                codeMaterial.Open();
                String vquery = "SELECT * FROM tblHistory WHERE Id = '" + dataGridView3.CurrentRow.Cells["Column2"].Value.ToString() + "'";
        ...
        {
            guna2Button10.FillColor = Color.DodgerBlue;
            guna2Button13.FillColor = Color.FromArgb(47, 49, 66);
        ...
        {
            guna2Button13.FillColor = Color.DodgerBlue;
            guna2Button10.FillColor = Color.FromArgb(47, 49, 66);
